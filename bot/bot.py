...
class PhraseBot(commands.Bot):
    def __init__(self):
        ...
        # BOMB GAME CONFIG
        # ----------------------------------------------------------------
        # Word list — chat must NOT say the secret word while the bomb is armed.
        # If they do, the bomb resets fully back to STATE_WAITING.
        self.word_list = [
            "apple",
            "banana",
            "shadow",
    ...
    def _arm_bomb(self):
        """Move from STATE_WAITING → STATE_ARMED with a new secret word."""
    ...
    async def current_word_cmd(self, ctx):
        ...
        shortage_after  = max(0, (idx + after + 1) - total)
        before += shortage_after
        after  += shortage_before
